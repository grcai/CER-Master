...
class ReplayBuffer(object):
	def __init__(self, state_dim, action_dim, max_size=int(18e4)):
	...
	def sample1(self, batch_size, temp_number):
		...
		for i in range(len(temp_number)-1):
			#ind1 = random.sample(range(temp_number[i], temp_number[i+1]), int(math.ceil(batch_size * (temp_number[i+1]-temp_number[i])/temp_number[len(temp_number)-1])))
			ind1 = random.sample(range(temp_number[i], temp_number[i+1]), int(math.ceil(batch_size / 3)))
	...
	def Choose_sample(self, result, k):
		index = []
		for i in range(k):
